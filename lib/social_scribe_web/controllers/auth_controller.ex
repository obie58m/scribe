defmodule SocialScribeWeb.AuthController do
  ...
  def callback(%{assigns: %{ueberauth_auth: auth, current_user: user}} = conn, %{
    ...
    case Accounts.find_or_create_user_credential(user, auth) do
      ...
      {:error, reason} ->
        Logger.error("Failed to save LinkedIn credential: #{inspect(reason)}")
  ...
  def callback(%{assigns: %{ueberauth_auth: auth}} = conn, _params) do
    Logger.info("Google OAuth login attempt for #{auth.info.email}")
  ...
  def callback(conn, _params) do
    Logger.error("OAuth callback received without auth data")
